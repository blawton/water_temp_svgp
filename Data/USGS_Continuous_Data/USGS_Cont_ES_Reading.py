...
with open("../../config_with_new_names.yml", mode='r', encoding="utf-8") as file:
    config = yaml.safe_load(file)
# -
#Local Data Source
output_file = "USGS_Cont_ES_Pre_Processing.csv"
stations_output = "USGS_Cont_ES_Stations.csv"
...
def get_stations(all_text):
    ...
    while not station_end_line:
        ...
        i+=1
        prev=i
        re1 = re.compile(r'\# *USGS')
        if re.match(re1, line) is not None:
            lines.append(line)
        elif len(lines)!=0:
            station_end_line=True
    re2 = re.compile(r'USGS (\d+) (\w+)')
    re3 = re.compile(r'USGS (\d+) ([\w ]+)')
    for line in lines:
        station_dict[re.search(re2, line).group(1)]= re.search(re3, line).group(2)
    return(station_dict)
