...
# Eventually dates will be removed on these:
input_file="STS_Continuous_Data_Pre_Processing.csv"
output_file="Interpolated_STS_Continuous_Data.csv"
pre_output_file="Processed_STS_Continuous_Data.csv"
...
years=pd.unique(pd.to_datetime(sts["Date Time (GMT-04:00)"], format='mixed').dt.year)
print(years)
...
#Graphing (latest notebook only graphs final output)
...
#Testing endpoint removal (latest notebook only graphs final output)
# for index, df in despiked.items():
#     for year in pd.unique(df["Year"]):
#         working=df.loc[df["Year"]==year]
#         plt.plot(working["Date"], working[dep_var], c="tab:blue")
...
#Testing interpolation (latest notebook only graphs pre-interpolated data)
